...
export const getMovieDetails = async (mediaType: string, titleId: number): Promise<IGetMovieDetailsResponse> => {
    ...
    const releaseDateApiResponse: ReleaseDatesResponse & Partial<ITMDBError> = await response.json();
    ...
    if (usRelease) {
        ...
        releaseInfo = releaseDateApiResponse.results.find((item) => {
          ...
          const hasValidCertification = item.release_dates.some(
            (rd: ReleaseDateEntry) => rd.certification && rd.certification.trim() !== ""
          );
