...
export const getPreviewDetails = async (
  ...
  let releaseDateApiResponse:
    | (ReleaseDatesResponse & Partial<ITMDBError>)
    | null = null;
  ...
  if (mediaType === "movie") {
    ...
    response = await fetch(
      `https://api.themoviedb.org/3/${mediaType}/${titleId}/content_ratings`,
      options
    );
